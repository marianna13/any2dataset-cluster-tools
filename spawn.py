...
def setup_node(
    ssh_key, host, username, master_addr, mem_gb=1, bash_file="setup_node.sh"
):
    ...
    client = create_ssh_client(host, username, ssh_key)
    stdin, stdout, stderr = client.exec_command(
        f"chmod +x /{username}/{bash_file_name}"
    )
    stdin, stdout, stderr = client.exec_command(
        f"bash /{username}/{bash_file_name} {master_addr} {mem_gb} ray"
    )
    while True:
...
def spawn_workers(
    ssh_key, hosts, username, master_addr, num_nodes, existing_nodes, bash_file
):
    print(len(hosts))
    ...
    for host_id, host in enumerate(hosts):
        ...
        print("=====================================")
        print(
            f"Setting up node {host}, {host_id+1+num_existing_nodes}/{num_nodes+num_existing_nodes}"
        )
        print("=====================================\n")
        if host == master_addr:
            continue
...
def spawn_one_worker(ssh_key, host, username, master_addr, mem_gb=1, bash_file=None):
    ...
    try:
        scp_file(ssh_key, host, username, mount_key, f"~/.ssh/mount_key")
        setup_node(
            ssh_key, host, username, master_addr, mem_gb=mem_gb, bash_file=bash_file
        )
...
def spawn_workers_parallel(
    ...
    with mp.Pool(num_proc) as pool:
        pool.starmap(
            spawn_one_worker,
            [
                (ssh_key, host, username, master_addr, mem_gb, bash_file)
                for host in hosts
            ],
        )
...
class Cluster:
    ...
    *_get_other_clusters_info(self) -> List[dict]* - gets info for each of already running clusters
    ...
    def __init__(
        self, num_workers, mem_gb, hosts, logs_dir, cluster_id=None, bash_file=None
    ):
    ...
    def kill_nodes(self, num_nodes):
        ...
        self._update_info(
            {
                "workers": self.workers,
                "num_workers": self.num_workers - num_nodes,
                "status": "running",
            }
        )
        self._save_info()
...
def main():
    ...
    cluster = Cluster(
        num_nodes, mem_gb, hosts, logs_dir, cluster_id, bash_file=bash_file
    )
    ...
    if action == "add_nodes":
        ...
        cluster_info = cluster._get_info()
        print(
            "Number of nodes in cluster after adding nodes: ",
